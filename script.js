...
const modalgweiTotalScoreEleme = document.querySelector("[modal-data-wei-total-score]")
const modalnftTotalScoreElem = document.querySelector("[modal-data-nft-total-score]")

const modalnftScoreElem = document.querySelector("[modal-data-nft-score ]")
const modalScoreTest = document.querySelector("[modal-data-score]")
...
function checkIfWeGotNft() {
  ...
  if(getNftRects().some(rect => isCollision(rect, playerRect))) {
    ...
    modalnftScoreElem.textContent = `nft score: ${nftScore}`
  }
...
function updateScore(delta) {
  ...
  modalScoreTest.textContent = `Wei score: ${Math.floor(score)}`
}
...
function handleLose() {
  ...
  //modalnftTotalScoreElem.textContent = `NFT total score: ${window.totalNFTScore}`
  gweiTotalScoreEleme.textContent = `Wei total score ${window.totalGweiScore}`
  //modalgweiTotalScoreEleme.textContent = `Wei total score ${totalGweiScore}`

  nftScore = 0
  nftScoreElem.textContent = `NFT Points Score: ${nftScore}`
 // modalnftScoreElem.textContent = `nft score: ${nftScore}`
  setPlayerLose()
  setTimeout(() => {
  ...
  modal.style.display='block';
    ...
    document.getElementById("totalNftScore").textContent = "NFT total score: " + totalNFTScore;
    document.getElementById("totalGweiScore").textContent = "Gwei total score: " + totalGweiScore;
